 ...
 * $URL: https://fhlnoip.googlecode.com/svn/trunk/ds/list/dlinklist/dlinklist.c $
 */
...
typedef struct _dlinklist_node {
    int _data;
    struct _dlinklist_node *_prev;
    struct _dlinklist_node *_next;
} NODE, *NODEPTR;

NODEPTR _dlinklist_create_node()
{
...
NODEPTR dlinklist_create()
{
    NODEPTR head = _dlinklist_create_node();
...
/*
int dlinklist_free(NODEPTR head)
{
	if (head != head->_next) {
		dlinklist_free(head->_next);
		head->_next == NULL;
	}
...
int dlinklist_insert(NODEPTR target, NODEPTR node)
{
}

int dlinklist_remove(NODEPTR node)
{
}
